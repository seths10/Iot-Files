...
horadc = ADC(Pin(34))  # create ADC object on ADC pin
horadc.atten(ADC.ATTN_11DB)
veradc = ADC(Pin(33))  # create ADC object on ADC pin
...
class Mario(object):
    ...
    def draw(self):
        ...
        if prev_x > x:
            # Left
            self.display.draw_filledRectangle(x + w, prev_y, 1, h, 0)
        elif prev_x < x:
            # Right
            self.display.draw_filledRectangle(x - 1, prev_y, 1, h, 0)
        if prev_y > y:
            # Upward
            self.display.draw_filledRectangle(prev_x, y + h, w, 1, 0)
        elif prev_y < y:
            # Downward
            self.display.draw_filledRectangle(prev_x, y - 1, w, 1, 0)
    ...
    def clear(self):
        """Clear sprite."""
        self.display.draw_filledRectangle(self.x, self.y, self.w, self.h, 0)
...
class Wall(object):
    ...
    def draw_wall(self, display):
        ...
        Args:
            display (SSD1351): OLED display object.
        """
        display.draw_filledRectangle(self.left, self.top, self.w, self.h, self.color)
